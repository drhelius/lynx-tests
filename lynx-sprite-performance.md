...
To render a list, the CPU gives Suzy the video-buffer address and the address of the first Sprite Control Block, starts the sprite process through `SPRGO`, and sleeps. The previous Suzy wakeup must also be acknowledged through `SDONEACK`. Atari's earlier launch list places this write after `SPRGO`, while its later Common Errors section requires acknowledgement before running the engine, including first use. The conservative sequence is to set `VIDBASE` and `SCBNEXT` and acknowledge the previous wakeup through `SDONEACK` before writing `SPRGO`, then execute `CPUSLEEP`. Sleeping is essential: Suzy shares system RAM with the CPU and Mikey, and it cannot make normal progress while the CPU retains the bus. An interrupt may wake the CPU before Suzy is finished; in that case software services the interrupt and sleeps again until the active bit in `SPRSYS` clears.
...
Focused Lynx I and Lynx II tests confirm that writing `SPRGO.B0` starts the sprite engine and sets sprite busy even while `SUZYBUSEN.B0` is clear. The operation waits for bus permission and CPU sleep before making progress. Completion leaves a persistent Suzy-done wake condition: repeated `CPUSLEEP` writes return immediately until software writes any value to `SDONEACK`. An enabled pending Mikey interrupt also prevents sleep regardless of the CPU interrupt-disable flag, while an interrupt that wakes the CPU during sprite work can be cleared and followed by another `CPUSLEEP` without acknowledging Suzy.
